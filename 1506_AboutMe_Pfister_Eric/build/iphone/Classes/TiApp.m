...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1506_AboutMe_Pfister_EricUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1506_AboutMe_Pfister_EricUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1506_AboutMe_Pfister_Eric app using the node.js CLI

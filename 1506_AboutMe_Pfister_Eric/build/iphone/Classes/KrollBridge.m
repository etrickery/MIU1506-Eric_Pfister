...
NSString * k1506_AboutMe_Pfister_Eric$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1506_AboutMe_Pfister_EricObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1506_aboutme_pfister_eric);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1506_aboutme_pfister_eric gc];
}
...
{
	// create k1506_AboutMe_Pfister_Eric global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1506_AboutMe_Pfister_Eric" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1506_aboutme_pfister_eric = [[k1506_AboutMe_Pfister_EricObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1506_aboutme_pfister_eric];
	
	NSString *_k1506_aboutme_pfister_ericNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1506_aboutme_pfister_ericNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1506_aboutme_pfister_eric valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1506_AboutMe_Pfister_Eric$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1506_aboutme_pfister_eric.kroll" 
